...
export const createDailyReportWorkflow =
  (reportRepo: DailyReportRepository, userRepo: UserRepository, projectRepo: ProjectRepository) =>
...
export const updateDailyReportWorkflow =
  (reportRepo: DailyReportRepository, projectRepo: ProjectRepository) =>
  async (input: UpdateDailyReportInput): Promise<Either<DomainError, DailyReport>> => {
    // 1. 既存日報の取得
...
export const submitDailyReportWorkflow =
  ...
  async (input: SubmitDailyReportInput): Promise<Either<DomainError, DailyReport>> => {
    // 1. 既存日報の取得
    const report = await reportRepo.findById(input.id)
    if (!report) {
      return left(notFound('日報が見つかりません'))
...
export const approveDailyReportWorkflow =
  ...
  async (input: ApproveDailyReportInput): Promise<Either<DomainError, DailyReport>> => {
    // 1. 既存日報の取得
    const report = await reportRepo.findById(input.id)
    if (!report) {
      return left(notFound('日報が見つかりません'))
...
export const rejectDailyReportWorkflow =
  (reportRepo: DailyReportRepository, userRepo: UserRepository) =>
  async (input: RejectDailyReportInput): Promise<Either<DomainError, DailyReport>> => {
    // 1. 既存日報の取得
    const report = await reportRepo.findById(input.id)
    if (!report) {
      return left(notFound('日報が見つかりません'))
...
export const createCommentWorkflow =
  (reportRepo: DailyReportRepository, userRepo: UserRepository, commentRepo: CommentRepository) =>
  async (input: CreateCommentInput): Promise<Either<DomainError, Comment>> => {
    // 1. 日報の存在確認
