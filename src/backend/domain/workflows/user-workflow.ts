...
export const createUserWorkflow =
  ...
  async (input: CreateUserInput): Promise<Either<DomainError, User>> => {
    ...
    if (!isStrongPassword(input.password)) {
      return left(
        validationError('パスワードは8文字以上で、大文字・小文字・数字を含む必要があります')
      )
    }
...
export const changePasswordWorkflow =
  (userRepo: UserRepository, passwordHasher: PasswordHasher) =>
  async (input: ChangePasswordInput): Promise<Either<DomainError, void>> => {
    // 1. ユーザーの取得
...
export const authenticateUserWorkflow =
  (userRepo: UserRepository, passwordHasher: PasswordHasher) =>
  async (input: AuthenticateUserInput): Promise<Either<DomainError, User>> => {
    // 1. メールアドレスでユーザーを検索
...
export const getUserWorkflow =
  (userRepo: UserRepository) =>
  async (userId: string): Promise<Either<DomainError, User>> => {
...
export const getSubordinatesWorkflow =
  (userRepo: UserRepository) =>
  async (managerId: string): Promise<Either<DomainError, User[]>> => {
    // 1. マネージャーの存在確認
